...
# When a release-specific index is not published yet (e.g. rhoai/2.25/cpu-ubi9),
# fall back to the newest known stable index that exists.
RHOAI_INDEX_FALLBACK_RELEASES = ("3.5", "3.4.ea.2", "3.4.ea.1")
...
def _resolve_from_release(
    ...
    *,
    ...
    """Fallback for non-AIPCC bases (e.g. UBI python-312): use RELEASE= from build-args."""
    release = entries.get("RELEASE")
    if not release:
        return None

    accelerator = flavor
    release_candidates = [release]
    for fallback in RHOAI_INDEX_FALLBACK_RELEASES:
        if fallback not in release_candidates:
            release_candidates.append(fallback)

    for release_candidate in release_candidates:
        production_url, test_url = index_url_candidates(release=release_candidate, accelerator=accelerator)
...
def resolve_index_config(
    ...
    *,
    ...
    if resolved := _resolve_from_release(entries, conf_file, flavor=flavor, product=product, base_image=base_image):
        return resolved
