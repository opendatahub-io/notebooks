...
#---------------------------------------------------------
main() {
    local start_dir="./jupyter"
    echo "Scanning $start_dir for directories containing Dockerfile.konflux.*"
    echo "Comparing Dockerfiles, ignoring comments and LABEL blocks..."

    # Populate array of directories
    mapfile -t docker_dirs < <(find_docker_dirs "$start_dir")
...
strip() {
    awk '
        ...
        in_label {
            if ($0 !~ /\\$/) in_label = 0  # End LABEL block if line does not end with backslash
            next
        }

        # Detect start of LABEL instruction
        /^[[:space:]]*LABEL([ \t]|$)/ {
            if ($0 ~ /\\$/) in_label = 1   # Multi-line LABEL block
            next
        }
