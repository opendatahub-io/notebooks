...
| ----------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `helpers/pylock-to-requirements.py`             | pip              | Convert `pylock.<flavor>.toml` (PEP 751) to pip-compatible `requirements.<flavor>.txt` with `--hash` lines. Default `--sdist-hashes el9-fallback` omits sdist hashes when an EL9-compatible wheel exists; the hermetic baseline uses `--sdist-hashes all` because it builds every package from source. |
