...
provided, it extracts RHEL entitlement certs and passes them to Hermeto for
`cdn.redhat.com` authentication.  Certs are obtained from the
`notebook-rpm-lockfile` image if available (local dev flow after running
`create-rpm-lockfile.sh`), otherwise a temporary UBI container is registered
with `subscription-manager` to obtain them (CI flow).
...
- `-v $(realpath ./cachi2/output):/cachi2/output:z` bind-mount the prefetched dependencies
