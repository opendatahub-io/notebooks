#!/usr/bin/env python3
"""Convert a pylock.toml (PEP 665) to a pip-compatible requirements.txt.
...
Why not just use pylock.toml directly?
  - pip and cachi2 don't support pylock.toml (PEP 665) yet.
...
Arguments:
    pylock.toml       Input lock file (PEP 665 format, generated by uv pip compile)
...
def main():
    ...
    # Parse the pylock.toml (PEP 665 format, requires Python 3.11+ for tomllib)
    ...
    for pkg in data.get("packages", []):
        ...
        # each with a different hash — all must be listed so pip accepts
