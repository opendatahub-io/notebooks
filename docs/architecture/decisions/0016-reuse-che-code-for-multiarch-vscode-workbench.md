...
che-code 3.30 image ships VS Code 1.116.0 (verified from its `product.json`),
...
|---|---|---|---|
| **ODH** | `quay.io/che-incubator/che-code:7.121.0` | amd64, arm64 | Public (no subscription) |
| **RHOAI** | `registry.redhat.io/devspaces/code-rhel9:3.30` | amd64, arm64, ppc64le, s390x | Red Hat subscription |
...
The tag mapping is not obvious: upstream `7.121.0` corresponds to downstream
`3.30` — both ship VS Code 1.116.0. To find the matching upstream tag for a
...
- **Dependency on Dev Spaces release cadence.** The wrapper image pins to a
  specific che-code tag (`3.30`). VS Code version updates come on the Dev
