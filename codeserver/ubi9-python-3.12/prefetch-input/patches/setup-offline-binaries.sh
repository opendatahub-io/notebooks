...
# GHA runners (16GB) can hit ENOMEM during npm ci postinstall when many lifecycle
# scripts spawn in parallel. Serialise script execution for GHA builds only.
if [[ "${GHA_BUILD:-false}" == "true" ]]; then
    npm config set --global foreground-scripts true
    npm config set --global maxsockets 1
    echo "GHA build: npm foreground-scripts=true maxsockets=1"
fi
