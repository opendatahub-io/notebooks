...
#   2. node-gyp headers: NPM_CONFIG_NODEDIR=/usr tells node-gyp to use system
#      Node.js headers (from nodejs-devel RPM) — no Electron or Node.js header
#      tarballs needed. Additional headers for VS Code remote (22.20.0) are still
#      cached for native module compilation targeting the bundled Node.js version.
...
#   5. Node.js runtime binary: copy to .vscode-offline-cache/ for bundling
#   6. Pre-populate .build/ caches so gulp tasks skip network downloads:
...
# node-gyp headers: NPM_CONFIG_NODEDIR=/usr (set in codeserver-offline-env.sh)
# directs node-gyp to use system Node.js headers from nodejs-devel RPM for all
# native module compilation. The env var takes precedence over remote/.npmrc's
# disturl/target=22.20.0. We keep the 22.20.0 headers cached as a safety net in
# case any build step explicitly bypasses NPM_CONFIG_NODEDIR.
for version in 22.20.0; do
...
# Playwright Chromium: not prefetched (only used for tests, not the build).
...
cp "${HERMETO_OUTPUT}/deps/generic/node-v22.20.0-linux-x64.tar.gz" "${VSCODE_OFFLINE_CACHE}/"
