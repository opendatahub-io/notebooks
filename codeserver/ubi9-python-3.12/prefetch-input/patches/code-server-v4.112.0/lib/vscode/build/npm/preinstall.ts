...
function installHeaders() {
	...
	// Hermetic builds set npm_config_nodedir=/usr (nodejs-devel RPM) so node-gyp uses
	// system headers instead of downloading Electron/Node header tarballs (offline).
	const nodedir = process.env.npm_config_nodedir || process.env.NPM_CONFIG_NODEDIR;
	if (nodedir) {
		console.log(`Using system Node.js headers from nodedir=${nodedir}; skipping node-gyp header download.`);
		return;
	}
