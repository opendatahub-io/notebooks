...
// ODH: @typescript/native-preview (tsgo) has no linux-ppc64/s390x binaries in the
// dev preview used by VS Code 1.112. Fall back to tsc on those arches.
...
export function spawnTsgo(projectPath: string, config: { taskName: string; noEmit?: boolean }, onComplete?: () => Promise<void> | void): Promise<void> {
	...
	const args = [compiler, '--project', projectPath, '--pretty', 'false'];
	if (config.noEmit) {
		args.push('--noEmit');
	} else {
	...
	return new Promise<void>((resolve, reject) => {
		child.on('exit', code => {
			...
			if (code === 0) {
				...
				reject(new Error(`${compiler} exited with code ${code ?? 'unknown'}`));
			}
		});
