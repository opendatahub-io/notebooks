...
// ODH: ppc64/s390x builds use tsc (slower) under QEMU; serialize extension streams to avoid OOM.
const LOW_MEMORY_ARCHES = new Set(['ppc64', 's390x']);

function mergeExtensionStreams(streams: Stream[]): Stream {
	return LOW_MEMORY_ARCHES.has(process.arch) ? es.concat(...streams) : es.merge(...streams);
}
...
function fromLocal(extensionPath: string, forWeb: boolean, _disableMangle: boolean): Stream {
	...
	if (hasEsbuild) {
		...
		if (LOW_MEMORY_ARCHES.has(process.arch)) {
			// ODH: skip noEmit typecheck during packaging on ppc64/s390x — saves memory under QEMU.
			input = fromLocalEsbuild(extensionPath, esbuildConfigFileName);
		} else {
			const typeCheckStreams = getBuildRootsForExtension(extensionPath).map(root => typeCheckExtensionStream(root, forWeb));
			input = es.merge(
				fromLocalEsbuild(extensionPath, esbuildConfigFileName),
				mergeExtensionStreams(typeCheckStreams),
			);
		}
		isBundled = true;
	} else {
...
function doPackageLocalExtensionsStream(forWeb: boolean, disableMangle: boolean, native: boolean): Stream {
	...
	const extensionStreams = localExtensionsDescriptions.map(extension => {
		return fromLocal(extension.path, forWeb, disableMangle)
			.pipe(rename(p => p.dirname = `extensions/${extension.name}/${p.dirname}`));
	});
	const localExtensionsStream = minifyExtensionResources(mergeExtensionStreams(extensionStreams));
