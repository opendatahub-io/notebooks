...
export async function findTextInFrames(page: Page, text: string): Promise<{ frame: Frame; index: number } | null> {
  ...
  for (let i = 0; i < frames.length; i++) {
    ...
    if (body && body.includes(text)) return { frame, index: i };
  }
  return null;
}
...
async function runBatch(code: string) {
  ...
  try {
    ...
    const mod = {} as { default: (...args: unknown[]) => Promise<unknown> };
    // This intentionally evaluates user-supplied TypeScript from the REPL.
    ...
    if ('log' in error && Array.isArray(error.log)) console.error('Call log:', error.log.join('\n'));
    process.exitCode = 1;
  } finally {
    await browser.close();
  }
...
async function runInteractive() {
  ...
  r.on('exit', () => {
    ...
    void browser.close().then(() => process.exit());
  });
}
