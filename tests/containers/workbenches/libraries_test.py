...
from tests.containers.conftest import get_image_metadata
from tests.containers.workbenches.workbench_image_test import grab_and_check_logs, WorkbenchContainer
...
class TestWorkbenchImage:
    ...
    def test_image_entrypoint_starts(self, subtests: pytest_subtests.SubTests, workbench_image: str) -> None:
        ...
        try:
            try:
                ...
                docker_utils.container_cp(container.get_wrapped_container(), unittests,
                                          "/opt/app-root/src/")
                image_metadata = get_image_metadata(workbench_image)
                ecode, stdout = container.exec([
                    "env", f"IMAGE={image_metadata.labels["name"]}",
                    "bash", "-c", "python3 /opt/app-root/src/libraries_testunits.py"
                ])
