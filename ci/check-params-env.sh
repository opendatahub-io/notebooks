...
for file_ in  "${PARAMS_ENV_PATH}" "${PARAMS_LATEST_ENV_PATH}"; do
    # params-latest.env contains only dummy placeholder values for N (latest/dev)
    # images — they are moving targets rebuilt on each merge and replaced at
    # deployment time. Skip image validation for this file.
    if [ "${file_}" = "${PARAMS_LATEST_ENV_PATH}" ]; then
        echo "Skipping image validation for '${file_}' (params-latest.env uses dummy placeholders)"
        echo "------------------------"
        continue
    fi
