...
kickoff-release:
	...
	run_rpm_step odh 3 "create ODH rpm lockfile (root prefetch input)" env -u SUBSCRIPTION_ACTIVATION_KEY -u SUBSCRIPTION_ORG CONTAINER_ENGINE=$(CONTAINER_ENGINE) ./scripts/lockfile-generators/create-rpm-lockfile.sh --rpm-input prefetch-input/odh/rpms.in.yaml; \
	run_rpm_step odh 4 "create ODH rpm lockfile (codeserver prefetch input)" env -u SUBSCRIPTION_ACTIVATION_KEY -u SUBSCRIPTION_ORG CONTAINER_ENGINE=$(CONTAINER_ENGINE) ./scripts/lockfile-generators/create-rpm-lockfile.sh --rpm-input codeserver/ubi9-python-3.12/prefetch-input/odh/rpms.in.yaml; \
	run_rpm_step rhds 5 "create RHDS rpm lockfile (root prefetch input)" env CONTAINER_ENGINE=$(CONTAINER_ENGINE) ./scripts/lockfile-generators/create-rpm-lockfile.sh --rpm-input prefetch-input/rhds/rpms.in.yaml; \
	run_rpm_step rhds 6 "create RHDS rpm lockfile (codeserver prefetch input)" env CONTAINER_ENGINE=$(CONTAINER_ENGINE) ./scripts/lockfile-generators/create-rpm-lockfile.sh --rpm-input codeserver/ubi9-python-3.12/prefetch-input/rhds/rpms.in.yaml; \
