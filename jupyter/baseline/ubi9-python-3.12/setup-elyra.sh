...
# Resolve the path once and preserve it as one shell argument.
jupyter_data_dir="$(jupyter --data-dir)"

# create the elyra runtime directory if not present
if [ ! -d "${jupyter_data_dir}/metadata/runtimes/" ]; then
  mkdir -p "${jupyter_data_dir}/metadata/runtimes/"
fi
# Set elyra runtime config from volume mount
if [ "$(ls -A /opt/app-root/runtimes/ 2>/dev/null)" ]; then
  cp -r /opt/app-root/runtimes/..data/*.json "${jupyter_data_dir}/metadata/runtimes/"
fi

# Set elyra runtime images json from volume mount
if [ "$(ls -A /opt/app-root/pipeline-runtimes/ 2>/dev/null)" ]; then
...
export PIPELINES_SSL_SA_CERTS="${PIPELINES_SSL_SA_CERTS:-/var/run/secrets/kubernetes.io/serviceaccount/ca.crt}"
