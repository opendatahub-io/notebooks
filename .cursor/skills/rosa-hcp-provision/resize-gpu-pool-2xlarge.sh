...
printf '%s\n' "    Pass --context \"\$CLUSTER_CONTEXT\" on every oc command — never rely on"
printf '%s\n' "    the ambient current-context (see SKILL.md's \"Critical: always pass"
printf '%s\n' "    --context\" section; set CLUSTER_CONTEXT=\$(oc config current-context)"
printf '%s\n' "    once, right after login, if you haven't already):"
printf '%s\n' "    (get -w streams forever and never exits on its own — use a bounded wait):"
printf '%s\n' "    oc --context \"\$CLUSTER_CONTEXT\" wait --for=condition=Ready node -l hypershift.openshift.io/nodePool=$NEW_POOL --timeout=600s"
printf '%s\n' "    NEW_NODE=\$(oc --context \"\$CLUSTER_CONTEXT\" get node -l hypershift.openshift.io/nodePool=$NEW_POOL -o json)"
...
printf '%s\n' "    # 1800s timeout, no early-exit on crash-loop — oc wait has no \"give up"
printf '%s\n' "    # early\" concept, so tail events in a second terminal if this seems to"
printf '%s\n' "    # be taking too long: oc --context \"\$CLUSTER_CONTEXT\" get events -n nvidia-gpu-operator -w"
